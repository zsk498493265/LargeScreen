...
public class CommonService {
    ...
    public void informJwAndPq(String content) {
        ...
        if(result.isSuccess()==false || result.getData().equals("片区")){
            Integer organId=getIdBySession();
            List<SysUser> users=userDao.getJwUserId(organId);
    ...
    public void informJwAndPqKey() {
        Result result=checkUserOrganType();
        //以前的需求  片区也可以发布政策
        if(result.isSuccess()==false || result.getData().equals("片区")){
            Integer organId=getIdBySession();
            List<SysUser> users=userDao.getJwUserId(organId);
            List<Map<Integer,Integer>> map=oldmanDao.getKeyAddNum();
            Map<Integer,Integer> keyMap=new HashMap<>();
            map.forEach(s->keyMap.put(s.get("key"),s.get("value")));
            List<Message> messageList=new ArrayList<>();
            for(SysUser sysUser:users){
                Message message=new Message();
                message.setUserId(sysUser.getId());
                message.setType(MessageTypeEnum.XT.getIndex());
                if(keyMap.containsKey(sysUser.getOrganId())){
                    message.setContent("重点老人进行了更新，新增老人数："+keyMap.get(sysUser.getOrganId())+"人");
                }else{
                    message.setContent("重点老人进行了更新，新增老人数：0人");
                }
                messageList.add(message);
            }
            messageDao.saveAllMessages(messageList);
        }
    }
    ...
    public void fillXq(XqRequest xqRequest, XqInterface xqInterface) {
        ...
        if(xqRequest!=null && xqRequest.getXq()!=null && xqRequest.getXq().length>0){
            xqIds=Arrays.asList(xqRequest.getXq());
        }else if(xqRequest!=null && xqRequest.getJw()!=null && xqRequest.getJw().length>0){
    ...
    public List<String> getPrevNys(int num) {
...
//            response.addHeader("Cache-Control", "no-cache");
    ...
    public boolean checkAuth(String type) {
        Integer organId = getIdBySession();
        if (organId == 0 || organId == null) {
            ...
            //居委不做限制
            if(organ.getOrganTypeId()==2)
                return true;
            switch (type) {
                case "info":
                    if (organ.getAuthQueryInfo() != 1) {
                        return false;
                    } else {
                        return true;
                    }
                case "integral":
                    if (organ.getAuthQueryIntegral() != 1) {
                        return false;
                    } else {
                        return true;
                    }
    ...
    public boolean checkAddUser(String password) {
        String regStr = "^([A-Z]|[a-z]|[0-9]|[`~!@#$%^&*()+=|{}':;',\\\\[\\\\].<>/?~！@#￥%……&*（）――+|{}【】‘；：”“'。，、？]){6,18}$";
