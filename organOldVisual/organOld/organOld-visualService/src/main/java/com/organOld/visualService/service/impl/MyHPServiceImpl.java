...
public class MyHPServiceImpl implements MyHPService {
    ...
    @Override
    public String getVisualOlds(VisualRequest visualRequest,BTableRequest bTableRequest){
        Page<Visual> page=comService.getPage(bTableRequest,"oldman_base");
        Visual visual = visualWrap.unwrap(visualRequest);
        comService.fillXq(visualRequest,visual);
        page.setEntity(visual);
        List<OldmanModel> oldmanList=visualViewDao.getByPage(page).stream().map(oldmanWrapper::wrapVisual).collect(Collectors.toList());
        Long size = visualViewDao.getSizeByPage(page);
        return comService.tableReturn(bTableRequest.getsEcho(),size,oldmanList);
    }
    @Override
    ...
    @Override
    public Result getAgeSex(VisualRequest visualRequest){
        Map<String,Visual> visualMap = visualWrap.unwrapAgeSex(visualRequest);
        String[] keys = {"numOfSixToSevenF","numOfSevenToEightF","numOfEightToNineF","numOfOverNineF","numOfSixToSevenM","numOfSevenToEightM","numOfEightToNineM","numOfOverNineM"};
        Map<String,Long> dataMap = new HashMap<>();
        Visual visual = new Visual();
        if(visualRequest.getDistrict() != null)
            comService.fillXq(visualRequest,visual);
        if(visualRequest.getOrganIdArray() == null)
            for(String key:keys){
                Visual visual1 = visualMap.get(key);
                visual1.setXqIds(visual.getXqIds());
                Long size = visualViewDaoT.getVisualSizeByView(visual1);
            ...
            for(String key:keys){
                Visual visual1 = visualMap.get(key);
                visual1.setXqIds(visual.getXqIds());
                Long size = visualViewDao.getVisualSizeByView(visual1);
                dataMap.put(key,size);
            }
        return new Result(true,dataMap);
    }
    ...
    @Override
    public Result getSexNum(VisualRequest visualRequest){
        Map<String,Visual> visualMap = visualWrap.unwrapSex(visualRequest);
        String[] keys = {"numOfMale","numOfFemale"};
        Map<String,Long> dataMap = new HashMap<>();
        Visual visual = new Visual();
        if(visualRequest.getDistrict() != null)
            comService.fillXq(visualRequest,visual);
    ...
    @Override
    public Result getCensusNum(VisualRequest visualRequest){
        Map<String,Visual> visualMap = visualWrap.unwrapCensus(visualRequest);
        String[] keys = {"numOfCensus","numOfNotCensus"};
        Map<String,Long> dataMap = new HashMap<>();
        Visual visual = new Visual();
        if(visualRequest.getDistrict() != null)
            comService.fillXq(visualRequest,visual);
    ...
    @Override
    public Result getDistrictNum(VisualRequest visualRequest){
        Visual visual = visualWrap.unwrap(visualRequest);
        Map<String,Long> dataMap = new HashMap<>();
